...
class LogAggregator
{
        ...
        $query = $this->generateQuery(
            ...
            implode(
                ...
                array(
                    ...
                    sprintf(
                        '%s AS ' . $this->db->quoteIdentifier('%d') . ' ',
                        $Generic->getSqlRevenue('SUM(log_conversion_item.quantity * log_conversion_item.price)'),
                        Metrics::INDEX_ECOMMERCE_ITEM_REVENUE
                    ),
                    sprintf(
                        '%s AS ' . $this->db->quoteIdentifier('%d') . ' ',
                        $Generic->getSqlRevenue('SUM(log_conversion_item.quantity)'),
                        Metrics::INDEX_ECOMMERCE_ITEM_QUANTITY
                    ),
                    sprintf(
                        '%s AS ' . $this->db->quoteIdentifier('%d') . ' ',
                        $Generic->getSqlRevenue('SUM(log_conversion_item.price)'),
                        Metrics::INDEX_ECOMMERCE_ITEM_PRICE
                    ),
                    sprintf(
                        'COUNT(distinct log_conversion_item.idorder) AS ' . $this->db->quoteIdentifier('%d') . ' ',
                        Metrics::INDEX_ECOMMERCE_ORDERS
                    ),
                    sprintf(
                        'COUNT(distinct log_conversion_item.idvisit) AS ' . $this->db->quoteIdentifier('%d') . ' ',
            ...
            // GROUP BY ...
            sprintf(
                "$ecommerceType, label, log_conversion_item.%s",
                $dimension
            ),
    ...
    public static function getSelectsFromRangedColumn($column, $ranges, $table, $selectColumnPrefix, $restrictToReturningVisitors = false)
    {
        ...
        $selectAs = $db->quoteIdentifier($selectColumnPrefix . 'General_NewVisits');
